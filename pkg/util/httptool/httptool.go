...
func Request(method string, url string, options ...Option) (httpStatusCode int, respBody []byte, err error) {
	...
	defer func() {
		if err != nil {
			logger.Error(reqOpts.ctx, "HTTP_REQUEST_ERROR_LOG", "method", method, "url", url, "body", reqOpts.data, "reply", respBody, "err", err)
		}
	}()
	// 创建请求对象
	...
	if dur >= 3000 { // 超过 3秒 返回, 记一条 Warn 日志
		logger.Warn(reqOpts.ctx, "HTTP_REQUEST_SLOW_LOG", "method", method, "url", url, "body", reqOpts.data, "reply", respBody, "err", err, "dur/ms", dur)
	} else {
		logger.Debug(reqOpts.ctx, "HTTP_REQUEST_DEBUG_LOG", "method", method, "url", url, "body", reqOpts.data, "reply", respBody, "err", err, "dur/ms", dur)
	}
...
func WithContext(ctx context.Context) Option {
	return optionFunc(func(opt *requestOption) (err error) {
		opt.ctx = ctx
		return
	})
}
